...
const MediaGallery = () => {
  ...
  const getMediaUrl = (urlPath) => {
    if (!urlPath) return "";
    let fullUrl = urlPath.startsWith("http")
      ? urlPath
  ...
  const fetchItems = async (path) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/v2/media/browse?path=${encodeURIComponent(path)}`,
      );
      ...
      <div className="gallery-content">
        ...
        {items.map((item, index) => (
          <div
            ...
            onClick={() =>
              item.isDir ? handleFolderClick(item.path) : setPreviewItem(item)
            }
          >
            <div
              className={`item-icon-container ${item.isDir ? "folder-icon" : "file-icon"}`}
            >
              {item.isDir ? (
                <Folder size={40} />
              ) : isImage(item.name) ? (
