...
export default function ObdLive({ theme }) {
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async (devId) => {
      ...
      try {
        const response = await fetch(
          `${OBD_API_URL}/api/logs/v1.0/${devId}/latest`,
        );
        if (response.ok) {
    ...
    const handleUpdate = (data) => {
      ...
      console.log(`📍 Received update for ${devId}:`, {
        lat,
        lng,
        speed: data.speed,
      });
    ...
    if (deviceData && mapRef.current && isOnline) {
      ...
      if (prevPosRef.current) {
        const dist = Math.sqrt(
          Math.pow(deviceData.lat - prevPosRef.current.lat, 2) +
            Math.pow(deviceData.lng - prevPosRef.current.lng, 2),
        );
        if (dist > 0.00001) {
          const dLon =
            (deviceData.lng - prevPosRef.current.lng) * (Math.PI / 180);
    ...
    <LoadScript googleMapsApiKey={GOOGLE_MAPS_API_KEY}>
      <div className={`obd-page-wrapper obd-live-container ${theme}`}>
            ...
            >
              <div
                style={{
                  ...
                  height: "8px",
                  borderRadius: "50%",
                  backgroundColor: statusColors[functionalStatus],
                }}
              ></div>
              {functionalStatus === "ONLINE"
                ? "Connected (Live)"
                : functionalStatus === "STANDBY"
                  ? "Standby (Engine Off)"
                  : "Offline / Waiting"}
            </div>
          </div>
          ...
          >
            ...
            <div
              style={{
                ...
                gap: "10px",
                opacity: functionalStatus === "OFFLINE" ? 0.6 : 1,
              }}
            >
              <StatCard
              ...
              <StatCard
                ...
                label="Speed"
                value={`${Math.round((deviceData?.speed || 0) * 0.621371)} mph`}
                theme={theme}
              />
            </div>
            ...
            >
              ...
              >
                <DetailRow
                  ...
                  label="Heading"
                  value={`${deviceData?.direction || 0}°`}
                />
                <DetailRow
                  icon={<Activity size={14} />}
                  label="Last Seen"
                  value={
                    deviceData?.time ? formatDeviceTime(deviceData.time) : "--"
                  }
                />
              </div>
            </div>
        ...
        <div className="obd-map-container">
          ...
          >
            {deviceData &&
              isFinite(deviceData.lat) &&
              isFinite(deviceData.lng) && (
                <Fragment>
                  <OverlayView
                    position={{ lat: deviceData.lat, lng: deviceData.lng }}
                    mapPaneName="overlayMouseTarget"
                    getPixelPositionOffset={() =>
                      getPixelPositionOffset(50, 50)
                    }
                  >
                    <div
                      style={{
                  ...
                  {selected && (
                    ...
                    >
                      <div style={{ padding: "5px" }}>
                        <strong>Device ID:</strong> {activeDeviceId}
                        <br />
                        <strong>Speed:</strong>{" "}
                        {Math.round((deviceData.speed || 0) * 0.621371)} mph
                      </div>
                    </InfoWindow>
                  )}
                </Fragment>
