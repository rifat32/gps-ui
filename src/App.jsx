...
import {
  Play, Pause, RotateCcw, MapPin, AlertCircle, Loader2,
  Calendar, Search, Clock, Navigation, Zap, ChevronDown
} from 'lucide-react';
...
export default function App() {
  ...
  const processAndSetData = (packets, start, end) => {
    ...
    const filtered = packets
      ...
      .map((p, index) => {
        const lat = p.detailed?.body?.latitude?.degrees || (p.detailed?.body?.latitude?.decimal / 1000000);
        const lon = p.detailed?.body?.longitude?.degrees || (p.detailed?.body?.longitude?.decimal / 1000000);
        const speed = p.detailed?.body?.speed?.decimal || 0;
        const mileage = p.detailed?.body?.mileage?.decimal ? (p.detailed.body.mileage.decimal / 10).toFixed(1) : 0;
  ...
  const stops = useMemo(() => {
    ...
    trackingData.forEach((p, idx) => {
      if (p.speed === 0) {
        if (!currentStop) {
          currentStop = { ...p, startTime: p.timestamp, stopIndex: idx, count: 1 };
        } else {
          currentStop.endTime = p.timestamp;
          currentStop.count++;
        }
      } else {
        if (currentStop && currentStop.count > 5) { // Only count as stop if it stays for some time
          // Revisit detection (~50m threshold)
          const isRevisit = result.some(prev =>
  ...
  const currentPos = trackingData[currentIndex] || { lat: 0, lng: 0, status: 'N/A', time: '00:00:00', speed: 0, mileage: '0km' };

  // Calculate rotation
  const getRotation = () => {
    if (currentIndex > 0 && trackingData[currentIndex] && trackingData[currentIndex - 1]) {
  ...
  if (loading) return <div style={styles.loaderContainer}><Loader2 size={48} className="animate-spin" color="#3b82f6" /><p>Loading GPS Data...</p></div>;
          ...
          <div style={styles.sidebarContent}>
            ...
            <div style={styles.section}>
              <div style={styles.inputGroup}>
                ...
                <input style={styles.textInput} placeholder="Input Name or IMEI No." defaultValue="JT8088985963" />
              </div>
            ...
            <div style={styles.section}>
              ...
              <div style={styles.calendarMini}>
                ...
                {['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'].map(d => <span key={d} style={styles.calDay}>{d}</span>)}
                {[...Array(31)].map((_, i) => (
                  <span key={i} style={{ ...styles.calDate, color: i + 1 === 7 ? '#3b82f6' : '#334155', fontWeight: i + 1 === 7 ? 'bold' : 'normal' }}>
                    {i + 1}
                  </span>
                ))}
              </div>
              <div style={styles.dateTimeField}>
                ...
                <input type="datetime-local" value={`${startDate}T00:00`} onChange={(e) => setStartDate(e.target.value.split('T')[0])} style={styles.dateTimeInput} />
              </div>
              <div style={styles.dateTimeField}>
                <label style={styles.fieldLabel}>*Time for end:</label>
                <input type="datetime-local" value={`${endDate}T23:59`} onChange={(e) => setEndDate(e.target.value.split('T')[0])} style={styles.dateTimeInput} />
              </div>
            </div>
            ...
            <div style={styles.section}>
              <div style={styles.controlRow}>
                ...
                <div style={styles.speedSliderContainer}>
                  ...
                  <input type="range" min="1" max="10" value={playbackSpeed} onChange={(e) => setPlaybackSpeed(parseInt(e.target.value))} style={styles.slider} />
                  <span style={styles.speedText}>Fast</span>
                </div>
              </div>
              <div style={styles.controlRow}>
                <span style={styles.label}>Stopped time:</span>
                ...
                <div style={styles.thresholdBadges}>
                  <div style={styles.inputBadgeWrapper}>
                    <input
                      ...
                      style={{ ...styles.speedInput, backgroundColor: SPEED_COLORS.low }}
                    />
                  ...
                  <div style={styles.inputBadgeWrapper}>
                    <input
                      ...
                      onChange={(e) => setThresholds({ ...thresholds, normal: parseInt(e.target.value) || 0 })}
                      style={{ ...styles.speedInput, backgroundColor: SPEED_COLORS.normal }}
                    />
                    <span style={styles.unit}>km/h</span>
                  </div>
                  <div style={styles.inputBadgeWrapper}>
                    <input
                      ...
                      onChange={(e) => setThresholds({ ...thresholds, over: parseInt(e.target.value) || 0 })}
                      style={{ ...styles.speedInput, backgroundColor: SPEED_COLORS.over }}
                    />
                    <span style={styles.unit}>km/h</span>
                  </div>
                </div>
                <div style={styles.legendContainer}>
                  <div style={styles.legendItem}><span style={{ ...styles.dot, backgroundColor: SPEED_COLORS.low }}></span><span style={styles.legendText}>lowSpeed</span></div>
                  <div style={styles.legendItem}><span style={{ ...styles.dot, backgroundColor: SPEED_COLORS.normal }}></span><span style={styles.legendText}>normal</span></div>
                  <div style={styles.legendItem}><span style={{ ...styles.dot, backgroundColor: SPEED_COLORS.over }}></span><span style={styles.legendText}>OverSpeed</span></div>
                  <div style={styles.legendItem}><span style={{ ...styles.dot, backgroundColor: SPEED_COLORS.critical }}></span><span style={styles.legendText}>OverSpeed (1.5)</span></div>
                  <div style={styles.legendItem}><span style={{ ...styles.dot, backgroundColor: '#f97316' }}></span><span style={styles.legendText}>Revisit</span></div>
                </div>
              </div>

              {/* Quick Date Selectors */}
              <div style={styles.quickDates}>
                <button onClick={() => setQuickDate('LastWeek')} style={styles.quickBtn}>Last...</button>
                <button onClick={() => setQuickDate('ThisWeek')} style={styles.quickBtn}>This...</button>
                <button style={styles.quickBtn}>Before...</button>
                <button onClick={() => setQuickDate('Yesterday')} style={styles.quickBtn}>Yester...</button>
                <button onClick={() => setQuickDate('Today')} style={styles.quickBtn}>Today</button>
              </div>

              <button
                style={{ ...styles.startBtn, backgroundColor: isPlaying ? '#ef4444' : '#3b82f6' }}
            ...
            <div style={styles.eventList}>
              {stops.map((stop, ix) => (
                <div key={ix} style={{ ...styles.eventItem, borderLeft: stop.isRevisit ? '4px solid #f97316' : 'none' }}>
                  <div style={{ ...styles.eventPin, backgroundColor: stop.isRevisit ? '#f97316' : '#3b82f6' }}>P{ix + 1}</div>
                  <div style={styles.eventDetail}>
                    <p style={styles.eventAddr}>{stop.isRevisit ? '🔄 REVISITED LOCATION' : 'London Borough of Barking and Dagenham...'}</p>
                    <p style={styles.eventTime}>{stop.startTime} - {stop.endTime || 'Ongoing'}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <main style={styles.mapArea}>
            ...
            {selectedPoint && (
              ...
              >
                <div style={styles.tooltip}>
                  <p><strong>No. :</strong> P1</p>
                  <p><strong>Device name :</strong> JT8088985963</p>
                  <p><strong>Speed :</strong> {selectedPoint.speed} km/h</p>
                  <p><strong>Remain :</strong> {selectedPoint.count > 1 ? `${Math.floor(selectedPoint.count / 60)}h ${selectedPoint.count % 60}m` : '0m'}</p>
                  <p><strong>Start :</strong> {selectedPoint.startTime || selectedPoint.timestamp}</p>
                  {selectedPoint.endTime && <p><strong>Stop :</strong> {selectedPoint.endTime}</p>}
                </div>
              </InfoWindow>
            )}
          </GoogleMap>
          ...
          <div style={styles.bottomStats}>
            ...
            <div style={styles.statsBar}>
              ...
              <button onClick={() => setCurrentIndex(0)} style={styles.resetBtn}><RotateCcw size={14} /> Reset</button>
            </div>
          </div>
        </main>
      </div>
